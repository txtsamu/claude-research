...
### OneTerm migrated (Wave 3, second service, 2026-08-24)

Bastion host / SSH-proxy tool, the most complex Wave 3 service so far: 6 podman containers in one shared pod (`mysql`, `redis`, `guacd`, `acl`-api, `oneterm`-api, and a custom-built dark-mode UI). Replicated as a single k8s Pod with all 6 containers (same pattern as the Suwayomi/FlareSolverr merge), preserving the podman pod's internal networking exactly.

**Custom UI image**: `localhost/oneterm-ui:dark` (a locally-built dark-mode fork, `txtsamu/oneterm` on GitHub, see `oneterm-dark-mode.md`) existed only in homelab's podman storage -- no registry had it, and this cluster's `docker-mirror`/`ghcr-mirror` services turned out to not actually be wired into containerd's trust config at all (checked directly via `talosctl get mc` -- no `machine.registries.mirrors` section exists on any node). Pushed the exact existing image (byte-identical, no rebuild) to `ghcr.io/txtsamu/oneterm-ui:dark` as a private package instead -- Talos already trusts real HTTPS registries with zero config changes needed, just a `kubernetes.io/dockerconfigjson` imagePullSecret (`ghcr-pull-secret`) using the existing `gh auth token`. Verified with a real throwaway-pod pull before building anything else.

**Real gotcha, AES key preservation**: `config.yaml`'s `auth.aes.key`/`iv` encrypt every stored asset credential (SSH passwords/keys) in the database -- migrated byte-for-byte identical into a Secret, flagged in a comment as never-regenerate (no recovery path if lost).

**Real gotcha, `AddHost` replication**: the podman pod had `AddHost=mysql:127.0.0.1` / `AddHost=redis:127.0.0.1`. Config files only reference `127.0.0.1` directly, but the *migrated database itself* has at least one stored reference to the literal hostname `mysql` -- surfaced as a real startup failure after loading real data: `Can't connect to MySQL server on 'mysql' ([Errno -2] Name does not resolve)`. Fixed with k8s `hostAliases` (`127.0.0.1 -> mysql, redis`), exactly replicating podman's `/etc/hosts` behavior rather than trying to hunt down every internal reference.

**Data migration**: same stop-source -> tar -> relay -> load-into-PVC discipline as every prior app (`mysql-data` ~196MB, `replay` ~15MB, `rdp` empty). Podman was stopped before the tar and never restarted, so this doubled as the final pre-cutover sync -- no second resync pass needed. Verified with real checks, not just pod health: real asset/account counts queried directly from the migrated MySQL (`3` assets, `3` accounts), UI serving the real dark-mode HTML, the ui->api nginx proxy path returning a correct `401 Unauthorized` on an unauthenticated protected endpoint (proves the whole chain -- nginx, proxying, and the API's own auth enforcement -- is genuinely working, not just "the pod is Running"), and a real SSH banner (`SSH-2.0-Go`, OneTerm's own Go SSH server, distinct from Forgejo's real system `sshd`) on the bastion port.

**Cutover**: LoadBalancer IP `192.168.50.232` (MetalLB pool now at `.220-.232`, 14 assigned). Only the Cloudflare Tunnel needed repointing (`bas.<PERSONAL_DOMAIN>`: `http://localhost:8666` -> `http://192.168.50.232:80`) -- same tunnel-only exposure pattern as Forgejo, no `.lan` Caddy route existed. Podman's 6 OneTerm services disabled and stopped. **Correction (caught during a post-Nextcloud full Caddy audit, 2026-08-24): a `.lan` Caddy route DID exist for OneTerm after all** (`oneterm.lan`) -- missed at cutover time because the inventory check searched the wrong path (`/etc/caddy/Caddyfile`, which doesn't exist on this host; the real path is `/root/caddy/Caddyfile`), silently returned zero matches, and was misread as "no route exists" instead of "file not found." Left pointing at the dead podman port (`127.0.0.1:8666`) for a while before being caught. Fixed: repointed to `192.168.50.232:80`, verified `200` via `--resolve`. Lesson: a `grep` with no matches on a config file is not proof the config doesn't exist -- always confirm the file itself was actually found first (a wrong path fails silently the same way genuine absence does).

### Nextcloud migrated (Wave 3, third service, 2026-08-24)

App + Postgres + Redis in one shared pod, matching the podman quadlet's networking exactly (`POSTGRES_HOST=127.0.0.1`, `REDIS_HOST=localhost` -- same shared-pod pattern as OneTerm/Suwayomi, not the Service-name pattern used for Forgejo).

**The 155GB of real user files needed no data copy at all**: `config.php`'s `datadirectory` already points at `/mnt/nextcloud/data` on TrueNAS NFS (`192.168.50.10:/mnt/data/nextcloud`), reused directly via a static NFS `PersistentVolume` -- the exact same pattern already used for `photos-nfs` (shared with Jellyfin/copyparty), which Nextcloud also mounts at `/mnt/photos` for its own external-storage integration. Only the three *local* volumes needed the usual tar migration: app code/config (~870MB), Postgres data (~455MB, same `PGDATA` subdirectory fix as Forgejo/OneTerm), and Redis (~28KB, essentially empty).

**Real discrepancy caught before it mattered**: the podman quadlet's env vars declare `POSTGRES_USER=nextcloud`, but the *actual* already-installed `config.php` (migrated wholesale on the app-data PVC) authenticates as a completely different role, `oc_admin`, with its own separate password -- environment variables only govern a *fresh* `initdb`/install, and are silently ignored once real data exists. Real data was being migrated either way, so this didn't need fixing, just flagging: `config.php` is the actual source of truth for a running Nextcloud, not the container's env vars.

**Verified for real, not just pod health**: `occ status` on the migrated instance reports `installed: true` (not a fresh install), `occ user:list` shows the real `admin` account, `/mnt/nextcloud/data` shows the real per-user data directories over NFS, and `/status.php` returns a genuine `200` with real version info through both exposure layers.

**Cutover**: LoadBalancer IP `192.168.50.233` (MetalLB pool now at `.220-.233`, 15 assigned). Both exposure layers existed and both got repointed -- Caddy's `nextcloud.lan` (`127.0.0.1:8181` -> `192.168.50.233:80`) and the Cloudflare Tunnel's `cloud.<PERSONAL_DOMAIN>` (`http://localhost:8181` -> `http://192.168.50.233:80`) -- both already listed in `config.php`'s `trusted_domains`, no app-side config change needed. Podman's `nextcloud-app`/`nextcloud-db`/`nextcloud-redis`/`nextcloud-pod` disabled and stopped.

### Immich migrated (Wave 3, fourth and final service, 2026-08-24)

Server + Postgres (special `vectorchord`/`pgvector`-enabled image, required for face/CLIP search) + Redis (Valkey) + machine-learning in one shared pod, matching podman's `DB_HOSTNAME=localhost`/`REDIS_HOSTNAME=localhost` networking.

**The real 173GB photo/video library needed no data copy**: podman's `/mnt/immich-upload` was already TrueNAS NFS (`192.168.50.10:/mnt/data/immich-upload`), reused directly via a static NFS PV -- same pattern as `nextcloud-nfs`/`photos-nfs`.

**Postgres was different from every other Wave 3 database**: podman ran it on a raw iSCSI block device (`/dev/sdb`, manually provisioned, ext4-formatted, mounted at the OS level) rather than a podman volume -- almost certainly because the `pgvector`/`vectorchord` extension wants real block I/O semantics that NFS can't safely provide for a database. No need to preserve that specific LUN: this cluster's `truenas-iscsi` StorageClass (`democratic-csi`) already provisions fresh iSCSI-backed PVCs dynamically for every other Wave 3 Postgres, so the same tar-based migration (~2.2GB, `PGDATA` subdirectory fix) worked here too, onto a freshly-provisioned PVC.

**JWT_SECRET preserved byte-for-byte** (same discipline as Forgejo/OneTerm's keys) -- signs every session/API token, would invalidate all of them if regenerated. The ML model cache (~786MB: CLIP, facial-recognition, OCR models) was also migrated, avoiding a slow re-download on first real use, though it isn't data-loss-sensitive the way the database is.

**Verified for real, not just pod health**: real API responses (`/api/server/version`, `/api/server/ping`), a correct `401`-class "Authentication required" on the protected `/api/users/me` (proves the migrated `JWT_SECRET` config is actually working, not just present), and direct SQL against the migrated Postgres -- **349,385 real assets** and the real user account, both confirmed present.

**Cutover**: LoadBalancer IP `192.168.50.234` (MetalLB pool now at `.220-.234`, 16 assigned). Both exposure layers repointed -- Caddy's `immich.lan` (`127.0.0.1:2283` -> `192.168.50.234:2283`) and the Cloudflare Tunnel's `photos.<PERSONAL_DOMAIN>` (`http://localhost:2283` -> `http://192.168.50.234:2283`). Podman's `immich-server`/`immich-machine-learning`/`immich-postgres`/`immich-redis`/`immich-pod` disabled and stopped.

## Wave 3 complete (2026-08-24)

All four Wave 3 services -- Forgejo, OneTerm, Nextcloud, Immich -- are now fully migrated, cut over, and verified with real data. Every migrated app across Waves 1-3 also has a `topologySpreadConstraint` keeping it balanced across the 3 workers. `homelab-vm`'s remaining podman workload is now down to non-migration-tracked personal tools (`headroom-proxy`, `hermes`, etc.) plus whatever infra was deliberately left out of scope (Pi-hole, TrueNAS itself, `proxcenter`/`virtualmin` -- the latter two removed from Caddy/Pi-hole entirely at the user's request as genuinely unused).

**Real miss caught and fixed post-cutover**: OneTerm's Caddy `.lan` route (`oneterm.lan`, later renamed to `bastion.lan` at the user's request) was missed during its own migration -- the original inventory check searched the wrong Caddyfile path (`/etc/caddy/Caddyfile`, which doesn't exist on this host; the real path is `/root/caddy/Caddyfile`), got zero matches, and that was misread as "no route exists" rather than "file not found." Left pointing at the dead podman port for a while before a full post-Nextcloud Caddy audit caught it. Worth remembering: a `grep` with no matches on a config file path is not proof the config doesn't exist.

**Real OneTerm follow-up fix**: interactive SSH sessions through OneTerm to an external asset showed noticeable lag despite identical raw TCP connect latency to a direct SSH connection (~45-49ms both). Root cause was genuine CPU throttling on the `api` container (`cpu.stat` showed `nr_throttled: 9`) -- the SSH-proxy hop does real per-keystroke crypto/websocket-framing work, and the original `1`-core limit was too tight under that load. Bumped to `2` cores; throttling stopped.


## homelab-vm fully decommissioned: everything moved to warp-vm (2026-08-25)

With Wave 3 complete, the only things left on `homelab-vm` were pieces that never belonged in Kubernetes: personal AI-agent tooling (hermes, evomem, camofox, telegram/tiktok bots) and core LAN infrastructure (Pi-hole, Caddy, the Cloudflare Tunnel client). All of it moved to `warp-vm` (bumped to 4 vCPU/8GB/40GB specifically for this), completing `homelab-vm`'s decommission.

### AI-agent tooling migrated
- **hermes** (gateway + MCP bridge): git clone pinned at the exact same commit, venv rebuilt with the exact frozen package set (minus a genuinely-unused DingTalk/Matrix dependency cluster that had a real version conflict with hermes's own pins), full `HERMES_HOME` state (2.7GB: `auth.json`, sessions, checkpoints, bundled Node runtime) migrated byte-for-byte via tar+relay. Real gotcha: the migrated database expects the literal hostname `mysql`/`redis` to resolve locally in some contexts -- not applicable here, but `f2` (TikTok scraping lib used by `tiktok-bot`, sharing hermes's venv) needed a separate no-deps install to avoid downgrading hermes's own pinned versions.
- **evomem**: statically-linked binary, trivial copy; real knowledge-base data (277MB) migrated, verified with a real search hit matching this very session's own transcript.
- **camofox-browser**, **claude-telegram-bot** (rebuilt via `poetry install`, needed a matching `claude` CLI + `rtk` binary copied over too), **proxmox-mcp-plus** (PyPI package, installed system-wide) -- all migrated and verified.
- **Claude Code environment**: full `~/.claude` (765MB: sessions, plugins, settings) + a fresh `claude` CLI install, verified plugins load correctly.
- **tiktok-bot**: migrated but **non-functional on warp-vm** -- fails at Python's `f2` library's own token-generation step (`msToken 内容不符合要求`, a TikTok anti-bot rejection) despite identical public IP to homelab. Isolated to a real environment difference: homelab's hermes-venv is Python 3.11, warp-vm's is 3.13 (Debian 13's default) -- likely a TLS/HTTP client fingerprint difference TikTok's anti-bot flags. Left running on homelab as a deliberate fallback pending a Python-3.11-specific fix; not part of the "everything moved" claim.

### LAN infrastructure migrated (the risky part)
- **Caddy**: copied `Caddyfile` + `data/` + `config/` *wholesale* -- confirmed the exact same trusted local CA fingerprint survived the move (verified via `openssl x509 -fingerprint` on both hosts before cutover), so no client had to re-trust anything.
- **Pi-hole**: copied both podman volumes wholesale (real blocklists, custom `.lan` entries, query history). Real, non-obvious bug found: **Pi-hole's `dns.listeningMode = "single"` does NOT actually restrict the socket bind to one interface** -- FTL still binds `0.0.0.0:53` wildcard regardless (confirmed directly via `ss`); the actual restrictive mode is `"BIND"` (dashboard label "Bind only on interface"). This distinction turned out to matter far beyond Pi-hole itself, see below.
- **WARP + the SOCKS5 proxy (microsocks)**: this pairing turned into the hardest part of the whole move. Cloudflare's WARP client, in *every* mode (confirmed via its own documented connection-sequence steps), unconditionally runs a `Validating DNS configuration` check that assumes it owns port 53 -- a hard, undocumented incompatibility with running any local DNS server (Pi-hole) on the same host, confirmed with Cloudflare's own support docs found to have zero guidance for this scenario. Real root cause of the `0.0.0.0:53` wildcard bind (found via the Pi-hole `BIND`-mode fix above) also explained why WARP, Incus's own bridge dnsmasq, and Pi-hole were all fighting for the same port simultaneously.
  - **Fix**: installed `incus` and ran WARP + microsocks inside a dedicated system container (`warp-box`) with its own network namespace, so its own internal `systemd-resolved` conflict is fully isolated from the host's Pi-hole. Container built by pushing already-cached `.deb` files directly from the host (`incus file push`) rather than re-downloading over this ISP's slow path (measured ~500 B/s at one point) -- cut a would-be multi-hour dependency install down to seconds.
  - Real, separate bugs hit and fixed along the way: `libnspr4`/`libnss3` and the full `nftables`/`libnftnl11`/`libjansson4` stack are genuine *runtime* dependencies of `warp-svc` (not just the optional GUI tray app) -- WARP fails to even start without them (`error while loading shared libraries`, then `Failed to start firewall: No such file or directory`). Container needed `security.privileged=true` for WireGuard/netfilter operations to actually forward packets (control-plane "Connected" status was reachable even unprivileged, but no real traffic flowed). The connectivity check itself needs to reach an **IPv6-only** Cloudflare hostname (`engage.cloudflareclient.com`) -- this network has no real IPv6 anywhere (confirmed: even the host itself only has a link-local `fe80::` address) -- worked around by downgrading to the previous cached WARP client version (`2026.4.1350.0`), which doesn't perform that specific check. Even once genuinely connected (verified: real `warp=on`/`gateway=on` trace responses, real WireGuard packet counters), a PMTUD blackhole on this ISP's path silently dropped larger response packets through the tunnel specifically -- fixed by lowering the bridge/container MTU to 1400.
  - The container's `microsocks` binds its own internal IP; an Incus `proxy` device forwards the host's original `192.168.50.200:1080` straight through, so **zero config changes were needed** in any of the apps already pointed at that address (Suwayomi, FlareSolverr, SearXNG, Open WebUI, DeepSeek/OneTerm).
- **WARP Zero Trust device policy**: this device's Service Mode was locked (`Allow Mode Switch: false`) under a Zero Trust device profile forcing `Traffic and DNS mode`. Needed a real dashboard change (Zero Trust -> Team & Resources -> Devices -> Device profiles -> the matching profile -> Service mode -> "Traffic only mode") since the available API token lacked Zero Trust Devices scope. Verified via `warp-cli settings` showing `Mode: TunnelOnly` and `Allow Mode Switch: true` after propagation (~a few minutes, not instant).
- **cloudflared**: statically-linked binary, trivial copy; same tunnel token re-run from warp-vm -- Cloudflare supports multiple simultaneous connectors per tunnel, so this came up *before* homelab's was stopped, a genuinely zero-downtime cutover. Only one ingress rule (`agent.<PERSONAL_DOMAIN>` -> hermes-gateway's own API server) was homelab-local; it needed zero changes since hermes-gateway is now co-located with cloudflared on the same host.

### Router DNS cutover (the actual risk point)
MikroTik `/ip dhcp-server network` and `/ip dns` both hard-coded `192.168.50.80` (the old Pi-hole) as the primary DNS server -- updated both to `192.168.50.200`. This fedora machine's own NetworkManager connection turned out to have a **manual static DNS override** (`ipv4.dns=192.168.50.80`) bypassing DHCP entirely, unrelated to the router change and found only because a DHCP-renewal test showed no effect -- updated separately. End-to-end verified from a real client post-cutover: `.lan` resolution correct, real internet DNS correct, and HTTPS `.lan` access returns a clean `200` **without `-k`** (proving the migrated CA is genuinely trusted, not just that the server responds).
