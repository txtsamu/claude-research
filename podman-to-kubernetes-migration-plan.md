...
**Cutover**: LoadBalancer IP `192.168.50.232` (MetalLB pool now at `.220-.232`, 14 assigned). Only the Cloudflare Tunnel needed repointing (`bas.<PERSONAL_DOMAIN>`: `http://localhost:8666` -> `http://192.168.50.232:80`) -- same tunnel-only exposure pattern as Forgejo, no `.lan` Caddy route existed. Podman's 6 OneTerm services disabled and stopped.
