...
**Progress (2026-09-10):** T1 (#9), T2 (#10, agenix), T3 (#11, DNS), T4 (#12, Caddy), T5 (#13, tunnel), T6 (#14, NetBird - WARP dropped, see its resolution comment), T7 (#15, evomem), T8 (#16, MCP trio), T13 (#21, k3s core platform), T14 (#22, low-risk apps batch), T15 (#23, suwayomi), T16 (#24, forgejo), T17 (#25, nextcloud), T18 (#26, immich), T11 (#19, headroom-proxy) done and closed — see each issue's resolution comment for exact commands, values, and real bugs hit. **All 5 app-redeploy tickets (T14-T18) are now done** - `home` (192.168.50.202) runs the full platform layer plus **12 real apps with live data verified** (bookstack, checkmk, copyparty, couchdb, crawl4ai, openwebui, searxng, uptime-kuma, suwayomi, forgejo, nextcloud, immich - 357,039 real asset rows confirmed in immich's postgres alone), plus Technitium DNS, Caddy, a fresh Cloudflare Tunnel, NetBird, the MCP trio, and evomem. **Flagged for the user's attention**: T16 found 4 forgejo repos with no GitHub mirror - `capstone_project_documents`, `novia-app`, `novia-model`, `novia-web-api` - worth a deliberate backup decision before `warp-vm` is ever decommissioned (T20).
...
Frontier as of this update: T12 checkmk agent (#20, host monitoring agent - not the checkmk *app* already on home's k3s from T14) is the only ticket left before T19 cutover (#27) and T20 decommission (#28).
