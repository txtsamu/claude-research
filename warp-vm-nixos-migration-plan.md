...
**Progress (2026-09-10):** T1 (#9), T2 (#10, agenix), T3 (#11, DNS), T4 (#12, Caddy), T5 (#13, tunnel), T6 (#14, NetBird - WARP dropped, see its resolution comment), T7 (#15, evomem), T8 (#16, MCP trio), T13 (#21, k3s core platform), T14 (#22, low-risk apps batch), T15 (#23, suwayomi), T16 (#24, forgejo), T17 (#25, nextcloud) done and closed — see each issue's resolution comment for exact commands, values, and real bugs hit. `home` (192.168.50.202) now has: k3s + full platform layer, Technitium DNS, Caddy, a fresh Cloudflare Tunnel, NetBird mesh membership, the MCP trio, evomem, and **11 real apps with live data verified** (bookstack, checkmk, copyparty, couchdb, crawl4ai, openwebui, searxng, uptime-kuma, suwayomi, forgejo, nextcloud). **Flagged for the user's attention**: T16 found 4 forgejo repos with no GitHub mirror - `capstone_project_documents`, `novia-app`, `novia-model`, `novia-web-api` - worth a deliberate backup decision before `warp-vm` is ever decommissioned (T20).

**Important process gotcha discovered after T14 closed** (read before touching T18 or any further app migration): migrating an app's Deployment to `home` breaks `warp-vm`'s own live Caddy the moment the old Deployment is scaled to 0, because Caddy still points at `warp-vm`'s now-empty Service IP. T14-T16 missed this and caused real `.lan` outages for bookstack/openwebui/copyparty/uptime-kuma/checkmk/suwayomi, caught only when the user noticed and reported it - fixed after the fact by repointing `warp-vm`'s `/root/caddy/Caddyfile` at `home`'s new IPs. **T17 did this correctly**: repointed Caddy as part of the same ticket, immediately after confirming the app healthy, not as an afterthought. Do this for every remaining app ticket that has a `.lan` Caddy route. Also: edit that live Caddyfile with `cp`/direct write, never `sed -i` - it's bind-mounted into the Caddy container and `sed -i` swaps the inode, silently breaking the mount (`caddy reload` then reports "config is unchanged" and keeps serving the old file) - `systemctl restart caddy.service` is the recovery if this happens again.

Every app-redeploy ticket (T14+) has `warp-vm`'s old copies genuinely stopped, not deferred - RWO iSCSI storage makes that unavoidable, unlike the service-only tickets (T3-T8) where every deferral is still outstanding, tracked against T19. The temp MetalLB pool has grown to `192.168.50.240-252` (started as `.240-249` in T13, expanded in T17 when it filled up) - check remaining capacity before assuming a `<pending>` LoadBalancer IP is a real bug.

Frontier as of this update: T9 camofox (#17), T11 headroom-proxy (#19), T12 checkmk (#20, host monitoring agent - not the checkmk *app* already on home's k3s from T14), T18 immich (#26, plan's "highest-risk stateful migration" per [[immich-random-stops-automount-root-cause]] - has an NFS-backed PVC too, same NFS client fix from T14/T17 already in place on home's k3s.nix).
