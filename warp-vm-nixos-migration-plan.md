...
**Progress (2026-09-10):** T1 (#9), T2 (#10, agenix), T3 (#11, DNS), T4 (#12, Caddy), T5 (#13, tunnel), T6 (#14, NetBird - WARP dropped, see its resolution comment), T7 (#15, evomem), T8 (#16, MCP trio), T13 (#21, k3s core platform), T14 (#22, low-risk apps batch) done and closed — see each issue's resolution comment for exact commands, values, and real bugs hit (stale root Nix eval cache needing `--refresh`, containerd's `/etc/localtime` bind-mount needing `time.timeZone` set, democratic-csi's `/var/iscsi` hostPath, `netbird up`'s daemon-addr default drifting by version, the T1-era virtio_scsi/DNS-resolver fixes, and T14's batch: k3s's network-policy controller crash-looping against MetalLB nftables + kube-proxy needing `proxy-mode=nftables` + NFS client support/PATH on k3s.service + a Rancher/MetalLB annotation gotcha when exporting Services). `home` (192.168.50.202) now has: k3s + full platform layer, Technitium DNS, Caddy (16 routes), a fresh Cloudflare Tunnel, NetBird mesh membership, the MCP trio, evomem, and **8 real apps with live data** (bookstack, checkmk, copyparty, couchdb, crawl4ai, openwebui, searxng, uptime-kuma - static PVs pointing at the same TrueNAS zvols, verified with actual data, not just "pod is Running"). T14 is the one ticket where `warp-vm`'s old copies are genuinely stopped (not deferred) - RWO iSCSI storage makes that an unavoidable part of a safe handoff, unlike every service-only ticket before it. Frontier as of this update: T9 camofox (#17), T11 headroom-proxy (#19), T12 checkmk (#20, host monitoring agent - not to be confused with the checkmk *app* now on home's k3s from T14), and T15-T18 app redeploys (#23-26, same static-PV-handoff pattern as T14, plus watch for NFS-backed PVCs on T17/T18 - photos-nfs already needed by copyparty, immich-nfs/nextcloud-nfs will need the same NFS client fix from T14).
