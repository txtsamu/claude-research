...
**Progress (2026-09-10):** T1 (#9), T2 (#10, agenix), T3 (#11, DNS), T4 (#12, Caddy), T5 (#13, tunnel), T6 (#14, NetBird - WARP dropped, see its resolution comment), T7 (#15, evomem), T8 (#16, MCP trio), T13 (#21, k3s core platform), T14 (#22, low-risk apps batch), T15 (#23, suwayomi), T16 (#24, forgejo) done and closed — see each issue's resolution comment for exact commands, values, and real bugs hit (stale root Nix eval cache needing `--refresh`, containerd's `/etc/localtime` bind-mount needing `time.timeZone` set, democratic-csi's `/var/iscsi` hostPath, `netbird up`'s daemon-addr default drifting by version, k3s's network-policy controller crash-looping against MetalLB nftables + kube-proxy needing `proxy-mode=nftables` + NFS client support/PATH on k3s.service + a Rancher/MetalLB annotation gotcha when exporting Services, and T16's stale-containerd-status scare on `warp-vm` - real processes had already exited, `crictl`/kubelet status was just stuck, no actual corruption). `home` (192.168.50.202) now has: k3s + full platform layer, Technitium DNS, Caddy, a fresh Cloudflare Tunnel, NetBird mesh membership, the MCP trio, evomem, and **10 real apps with live data verified** (bookstack, checkmk, copyparty, couchdb, crawl4ai, openwebui, searxng, uptime-kuma, suwayomi, forgejo). **Flagged for the user's attention**: T16 found 4 forgejo repos with no GitHub mirror - `capstone_project_documents`, `novia-app`, `novia-model`, `novia-web-api` - worth a deliberate backup decision before `warp-vm` is ever decommissioned (T20). Every app-redeploy ticket (T14+) has `warp-vm`'s old copies genuinely stopped, not deferred - RWO iSCSI storage makes that unavoidable, unlike the service-only tickets (T3-T8) where every deferral is still outstanding, tracked against T19. Frontier as of this update: T9 camofox (#17), T11 headroom-proxy (#19), T12 checkmk (#20, host monitoring agent - not the checkmk *app* now on home's k3s from T14), T17 nextcloud (#25), T18 immich (#26, plan's "highest-risk stateful migration" per [[immich-random-stops-automount-root-cause]]) - both T17/T18 have NFS-backed PVCs (nextcloud-nfs/immich-nfs), same NFS client fix from T14 already in place on home's k3s.nix.
